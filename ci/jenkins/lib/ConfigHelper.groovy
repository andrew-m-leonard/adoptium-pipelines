 ...
 */
Map generatePipelineConfig(String configRepoPath = './config-repo') {
    ...
    if (params.RELEASE_TYPE) {
        String releaseType = params.RELEASE_TYPE.toUpperCase()
        List validReleaseTypes = ['NIGHTLY', 'WEEKLY', 'RELEASE']
 ...
 */
Map generateJenkinsConfig(String configRepoPath = './config-repo') {
    ...
    Map jenkinsConfig = readJSON(file: 'jenkins-config.json')
    ...
    Map labelsMap = jenkinsConfig.resolvedStageAgentLabels ?: jenkinsConfig.stageAgentLabels ?: [:]
    List labelPairs = labelsMap.collect { k, v ->
        "\"${k.replace('"', '\\"')}\":\"${v.replace('"', '\\"')}\""
    }
 ...
 */
void summarizePipelineConfig(Map pipelineConfig, Map jenkinsConfig = null) {
    ...
    if (jenkinsConfig) {
        ...
        (jenkinsConfig.resolvedStageAgentLabels ?: [:]).each { String stage, String label ->
            echo "  ${stage}: ${label}"
        }
