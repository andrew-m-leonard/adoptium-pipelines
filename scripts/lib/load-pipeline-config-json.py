...
def load_configuration(args):
    ...
    if release_type not in valid_release_types:
        print(
            f"ERROR: Invalid release type '{args.release_type}'. "
    ...
    if not adoptium_cfg_path.exists():
        print(
            f"ERROR: adoptium_pipeline_config.json not found at: {adoptium_cfg_path}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if missing_defaults:
        print(
            f"ERROR: Required fields missing from adoptium_pipeline_config.json: "
    ...
    if platform_key not in build_configurations:
        legacy_key = next(
            (
                old
                for old, new in LEGACY_PLATFORM_KEY_MAP.items()
                if new == platform_key
            ),
            None,
        )
        if legacy_key and legacy_key in build_configurations:
            print(
                f"Warning: config uses legacy platform key '{legacy_key}'; "
            ...
            print(
                f"ERROR: Platform '{platform_key}' not found in configuration.",
                file=sys.stderr,
            )
    ...
    configure_args = extract_variant_value(
        platform_config.get("configureArgs"), variant
    )
    ...
    additional_node_labels = extract_variant_value(
        platform_config.get("additionalNodeLabels"), variant
    )
    ...
    node_label = build_node_label(
        build_label_template, additional_node_labels, target_os, architecture
    )
    ...
    pipeline_config = {
        ...
        "parameters": {
            "cleanWorkspaceAfterStage": platform_config.get(
...
Examples:
  ...
  python3 load-pipeline-config-json.py \\
    ...
    # ── Required ──────────────────────────────────────────────────────────────
    parser.add_argument(
        "--jdk-version", required=True, help="JDK version (e.g., jdk21u, jdk17u)"
    )
    parser.add_argument(
        "--variant", required=True, help="Build variant (temurin, openj9, hotspot)"
    )
    parser.add_argument(
        "--target-os", required=True, help="Target OS (mac, linux, windows, aix)"
    )
    parser.add_argument(
        "--architecture",
        required=True,
