...
1. **Restart Behavior**: When restarting from a stage, the workspace will be cleaned before that stage executes
...
1. **Test Restart**: Restart from various stages to verify pre-cleanup works
1. **Test with Cleanup Disabled**: Set `CLEAN_WORKSPACE_AFTER_STAGE=false` to verify post-cleanup is skipped
1. **Test on Different Nodes**: Verify stages can run on different nodes without issues
