...
1. **AQA Tests** — runs AQAVit test suite (gated: runs only when `RUN_TESTS=true`)
1. **Reproducible Compare** — downloads Adoptium production binary and compares (gated: runs only when `RUN_REPRODUCIBLE_COMPARE=true` and `SCM_REF` is non-empty)
