...
- **Job-creation settings** (`jenkinsfilePath`, `pipelineTimeoutHours`, `pipelineBaseFolder`, `jobConfiguration`) — used by the seed job and launch job to configure Jenkins job definitions. Not needed at build runtime.
...
**Consumed by**: `scripts/lib/load-pipeline-config-json.py` (invoked by `ConfigHelper.groovy` on Jenkins, or `run-pipeline.py` locally)
...
The file is located by [`scripts/lib/load-pipeline-config-json.py`](../scripts/lib/load-pipeline-config-json.py) which constructs the path as:
...
`load-pipeline-config-json.py` detects which form is used, extracts the value for the active variant, and falls back to `"default"` if the variant key is absent.
...
**Generated by**: `scripts/lib/load-pipeline-config-json.py` via `ConfigHelper.generatePipelineConfig()` during the Initialize stage  
...
Build Pipeline (Jenkinsfile.declarative)
  ...
  │       runs scripts/lib/load-pipeline-config-json.py             ← merges params + per-version JSON
...
run-pipeline.py
  ...
  │     runs scripts/lib/load-pipeline-config-json.py              ← merges CLI args + per-version JSON
...
`load-pipeline-config-json.py` logs a warning and falls back to `"default"` when the active variant key is absent from a variant-specific object. Add the missing key or set a `"default"` entry.
